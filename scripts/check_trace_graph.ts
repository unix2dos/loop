const {conversationHeads, conversationID, conversationOverview, conversationTitle, shortTitle, traceKey} = await import(new URL("../web/dist/conversation.js", import.meta.url).href) as typeof import("../web/src/conversation.ts");
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
const { buildTraceGraph, relatedEvents, sameJSON, tokenUsage, runUsage, formatDuration, executionSections, timelineLayout } = await import(new URL('../web/dist/trace-graph.js', import.meta.url).href) as typeof import('../web/src/trace-graph.ts');
...
const event = (id: string, kind: string, turn: number, input: any, output: any, status = 'succeeded'): any => ({ id, kind, turn, input, output, status, title: kind, t: 0, d: 0, code: 'loop', explanation: '' });
...
check('对话侧栏合并连续轮次，旧运行保持独立', () => {
 const root: any={id:'root',created_at:1}, next={id:'next',conversation_id:'root',parent_run_id:'root',conversation_turn:2,created_at:2}, old={id:'legacy',created_at:0};
 assert.deepEqual(conversationHeads([next,root,old]).map(x=>x.id),['next','legacy']);
 assert.equal(conversationID({...old,task:"",model:"",status:"completed"}),'legacy');
...
check('未返回或非法 token 不当成零，部分统计保留覆盖数', () => {
 ...
 assert.equal(tokenUsage(event('e1','model',1,{}, {usage:{total_tokens:.5}})).total,undefined);
...
check('全程分组覆盖每一条真实事件，顺序不变',()=>{
 ...
 assert.deepEqual(groups.flatMap(g=>g.events.map((e: {id: string})=>e.id)),run.events.map((e: {id: string})=>e.id));
...
check('步骤和耗时概览都覆盖完整事件，运行中不补未来步骤',()=>{
 const run=example();run.events.forEach((e: any,i: number)=>{e.t=i*.2;e.d=.1});run.duration=1.2;
 for(const mode of ['steps','time'] as const){
  const layout=timelineLayout(run,mode);
  assert.deepEqual(layout.bars.map(b=>b.event.id),run.events.map((e: {id: string})=>e.id));
 ...
 assert.equal(executionSections({...example(),events:[]}).length,0);
});
