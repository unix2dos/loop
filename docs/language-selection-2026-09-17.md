...
日期：2026-09-17。状态：用户已认可 Go 内核 + TypeScript 网页路线，已记录为 [ADR-0001](adr/0001-go-runtime-typescript-ui.md)。实际迁移尚未开始；本轮调研没有修改应用、切换语言或重启暂停的服务。
...
上轮本机服务超时仍未诊断，不能把迁移作为已知修复。语言方向已经成为架构决策，应用实现尚未迁移。
