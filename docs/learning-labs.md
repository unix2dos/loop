...
**连接代码。** 在源码页找到 `agent.go` 的 `RunLoop`：外层循环限制模型请求次数，内层循环处理本批所有工具调用。不要把一个 TOOL 事件当作下一次模型请求。
...
**连接代码。** `tools.go` 的 `ExecuteReadonly` 返回可交给模型的错误对象；`trace.go` 记录实际结果，`agent.go` 将它作为配对的工具消息追加。
...
**连接代码。** `conversation.go` 读取并校验上一轮消息；`trace.go` 在已有消息末尾追加新的用户输入。原始事件的 `turn` 仍是模型请求计数，`conversation_turn` 才是用户对话轮次。
