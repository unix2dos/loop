...
export const traceKey = (runID: string, eventID: string): string => `${runID}:${eventID}`;
...
export function conversationOverview(runs: Run[], now = Date.now() / 1000) {
 ...
 const turns = runs.map((run,index) => {
  ...
  duration += seconds; eventCount += run.events.length;
  return turn;
 });
