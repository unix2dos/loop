...
const roles = { input: "INPUT", model: "MODEL", tool: "TOOL", control: "HARNESS" };
const lanes = { input: "Input", model: "Model", tool: "Tools", control: "Harness" };
const statuses = { running: "运行中", completed: "运行已结束", budget_exhausted: "模型请求额度耗尽", failed: "运行失败" };
let config = null;
let run = null;
let activeId = null;
let selected = null;
...
function summary(event) {
    ...
    if (typeof output.run_status === "string" && output.run_status in statuses) {
        return statuses[output.run_status] + " · 任务结果尚未验收";
    }
    return event.explanation;
}
function duration(event, current) {
...
function render() {
    ...
    if ($("follow").checked && events.length)
        selected = events[events.length - 1].id;
    ...
    $("lanes").innerHTML = Object.keys(lanes).map(kind => '<div class="lane kind-' + kind + '"><span class="lane-name">' + lanes[kind] + '</span><div class="track">' +
        events.filter(event => event.kind === kind).map(event => '<button class="bar ' + event.status + (event.id === selected ? " selected" : "") +
            '" data-event="' + esc(event.id) + '" title="' + esc(event.title) + '" aria-label="' + esc(event.title) +
            '" style="left:' + event.t / scale * 100 + "%;width:" + Math.max(.35, duration(event, current) / scale * 100) + '%"></button>').join("") + "</div></div>").join("");
    const filtered = events.filter(event => (event.title + " " + summary(event) + " " + roles[event.kind]).toLowerCase().includes(query));
    $("events").innerHTML = filtered.length ? filtered.map(event => '<button class="event kind-' + event.kind + " " + event.status + (event.id === selected ? " selected" : "") +
        ...
        (event.status === "running" ? "进行中" : event.d.toFixed(2) + "s") + "</span></button>").join("") : '<div class="empty">没有匹配的事件。</div>';
...
function renderDetail(event, current) {
    if (!event) {
    ...
    const tabs = [["overview", "这步发生了什么"], ["io", "输入 / 输出"], ["code", "核心代码"]];
    ...
    if (!source) {
        $("detail-content").textContent = "该历史事件没有源码快照。";
        return;
    }
    let content = "";
...
async function refreshHistory() {
    ...
    $("history").innerHTML = items.length ? items.map(item => '<option value="' + esc(item.id) + '">' + esc(statuses[item.status] + " · " + item.task.slice(0, 24)) + "</option>").join("") : '<option value="">尚无运行记录</option>';
...
async function initialize() {
    try {
        ...
        if (!config.configured)
            showError("请设置服务端的 OPENAI_API_KEY、OPENAI_MODEL，以及可选 OPENAI_BASE_URL。");
