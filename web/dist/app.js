...
function savePreference(key, value) { try {
...
let maxRequests = readBudget(), settingsReturn = "home";
let followLatest = true, followScrollLock = 0, renamingId = null;
...
function loadTitles() {
    try {
        const parsed = JSON.parse(localStorage.getItem("loop.conversationTitles") || "{}");
...
function saveTitle(id, value, task) {
    const titles = loadTitles(), next = value.replace(/\s+/g, " ").trim();
    if (!next || next === shortTitle(task))
        delete titles[id];
    else
        titles[id] = Array.from(next).slice(0, 40).join("");
    try {
        localStorage.setItem("loop.conversationTitles", JSON.stringify(titles));
    }
    catch { /* Browser storage is optional. */ }
}
function rootTask(item) { return items.find(root => root.id === conversationID(item))?.task ?? item.task; }
function displayTitle(item) { return conversationTitle(rootTask(item), loadTitles()[conversationID(item)] ?? ""); }
let conversationShare = .65;
try {
    const saved = Number(localStorage.getItem("loop.conversationShare"));
    if (Number.isFinite(saved) && saved > 0 && saved < 1)
        conversationShare = saved;
}
catch { /* Browser storage is optional. */ }
const splitter = $("trace-resize"), workbench = $("workbench");
function updateSplitWidth(share) {
    if (!splitter.offsetWidth)
        return; // Hidden in collapsed and phone layouts.
    ...
    if (share !== undefined)
        conversationShare = left / available;
...
splitter.onpointermove = event => {
    ...
    const available = workbench.clientWidth - splitter.offsetWidth;
    if (available > 0)
        updateSplitWidth((event.clientX - workbench.getBoundingClientRect().left - dragOffset) / available);
};
splitter.onpointerup = event => { if (splitter.hasPointerCapture(event.pointerId))
...
function updateURL(id, fresh = false, settings = false) {
...
function renderTaskLists() {
    ...
    if (!editing) {
        setHTML("task-list", heads.map(item => {
            ...
            if (id === renamingId)
                return `<div class="task-item ${active ? "active" : ""} renaming"><input id="task-title-input" class="task-title-input" value="${esc(label)}" maxlength="40" aria-label="对话标题"><span><i class="task-dot ${esc(item.status)}"></i><time>${date(item)}</time></span></div>`;
...
function finishRename(id, value) {
    ...
    if (value !== null && item)
        saveTitle(id, value, rootTask(item));
    renderTaskLists();
...
function showNewTask(changeURL = true) {
    page = "new";
    resetRunView();
    showError("");
    if (changeURL)
    ...
    if (!config?.configured)
        showError("请先在服务端配置模型，然后重启。");
    $("task").value = "";
    $("task").focus();
    render();
    void pollTaskList();
}
function showSettings(changeURL = true) {
    if (page !== "settings")
        settingsReturn = page;
    page = "settings";
    if (changeURL)
...
function leaveSettings() {
    if (settingsReturn === "run" && activeId && run) {
        page = "run";
        applyLayout();
        render();
        updateURL(activeId);
    }
    else if (settingsReturn === "new") {
        page = "new";
        applyLayout();
        render();
        updateURL(null, true);
...
async function pollTaskList() {
    try {
        ...
        if (page !== "run") {
            ...
            window.clearTimeout(pollTimer);
            if (busy)
                pollTimer = window.setTimeout(() => { void pollTaskList(); }, 1200);
        }
    }
    catch (error) {
        $("home-note").textContent = message(error);
        if (page === "new")
...
async function routeFromLocation() {
    ...
    if (params.has("run"))
        await chooseRun(params.get("run") ?? "", false);
...
function title(event) {
    ...
    if (event.kind === "tool")
        return { read_file: "读取文件", list_files: "列出文件", write_file: "修改文件", run_command: "运行测试" }[event.title] ?? event.title;
    return event.title;
}
...
function rowText(event) {
    ...
    if (event.kind === "tool") {
        if (event.title === "run_command")
            return `${event.output?.command ?? "node --test average.test.ts"} → ${event.output?.timed_out ? "执行超时" : event.output?.exit_code !== null && event.output?.exit_code !== undefined ? "退出码 " + event.output.exit_code : "退出状态未确认"}`;
        if (event.title === "write_file")
            return `${event.output?.path ?? "average.ts"} → ${event.output?.error ?? (event.output?.changed ? "已修改 · 查看真实 diff" : "内容未变化")}`;
    ...
    if (typeof event.input.system === "string") {
        ...
        if (typeof runtime.date === "string")
            return `本轮上下文 · ${runtime.date} · ${String(runtime.time_zone ?? "")} · ${event.input.system_refreshed ? "更新系统规则，保留历史对话" : "新对话"} · 模型请求额度 ${String(event.input.max_requests ?? "—")}`;
...
function renderDetail(current) {
    ...
    if (detailTab === "overview") {
        ...
        if (event.title === "write_file" && typeof event.output?.diff === "string")
            content += `<section><h3>实际文件变更</h3><pre>${esc(event.output.diff)}</pre></section>`;
        if (event.title === "run_command" && event.output)
            content += `<section><h3>测试回执 · ${event.output.timed_out ? "超时" : "退出码 " + esc(event.output.exit_code ?? "未确认")}</h3><pre>${esc(event.output.stdout || "（stdout 为空）")}</pre>${event.output.stderr ? `<pre>${esc(event.output.stderr)}</pre>` : ""}${event.output.output_truncated ? "<p>输出超过上限，已截断。</p>" : ""}</section>`;
...
async function loadConversation(current) {
    ...
    while (id) {
        if (seen.has(id))
            throw new Error("对话记录存在循环引用，无法载入");
        seen.add(id);
        let item = runCache.get(id);
        if (!item || (item.status === "running" && item.id !== current.id))
            item = await api("/api/runs/" + encodeURIComponent(id));
        if (conversationID(item) !== rootID)
            throw new Error("对话记录的关联不一致");
        runCache.set(id, item);
        chain.unshift(item);
        id = item.parent_run_id;
    }
    return chain;
}
...
function render() {
    ...
    if (followLatest && head.status === "running") {
...
async function poll() {
    ...
    const id = activeId;
    try {
        const snapshot = await api("/api/runs/" + encodeURIComponent(id));
        if (activeId !== id)
            return;
        if (!run)
            followLatest = snapshot.status === "running";
        run = snapshot;
        await refreshHistory();
...
document.addEventListener("click", event => {
    if (!(event.target instanceof Element))
        return;
    const rename = event.target.closest("[data-rename]")?.dataset.rename;
    if (rename) {
        startRename(rename);
        return;
    }
    ...
    if (event.target.closest("[data-leave-settings]")) {
        leaveSettings();
        return;
    }
    if (event.target.closest("[data-toggle-trace]")) {
        traceCollapsed = !traceCollapsed;
        savePreference("loop.traceCollapsed", traceCollapsed);
        applyLayout();
        render();
    }
...
$("graph-scroll").addEventListener("scroll", () => {
    if (!followLatest || followScrollLock || page !== "run")
        return;
    const head = conversation.at(-1);
...
$("budget").onchange = () => {
    ...
    if (n >= 1 && n <= 8) {
        maxRequests = n;
        savePreference("loop.maxRequests", n);
    }
};
window.addEventListener("popstate", () => { void routeFromLocation(); });
$("task-form").addEventListener("submit", async (event) => {
    event.preventDefault();
    if (!config || busy || (page === "run" && !conversationReady))
        return;
    showError("");
    busy = true;
    ...
    try {
        const result = await api("/api/runs", { method: "POST", headers: { "Content-Type": "application/json", "X-Lab-Token": config.token }, body: JSON.stringify({ task: $("task").value, max_requests: maxRequests, ...(page === "new" && newExercise ? { exercise: "ts-average", approve_exercise: true } : {}), ...(page === "run" && conversation.length ? { parent_run_id: conversation.at(-1).id } : {}) }) });
        $("task").value = "";
        activeId = result.id;
        await refreshHistory();
...
async function initialize() {
    try {
        ...
        $("home-note").textContent = "无法连接本机服务：" + message(error);
    }
}
void initialize();
