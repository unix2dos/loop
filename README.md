...
“步骤”刻度帮助看清全程，“耗时”刻度按实际时间分布。点击概览色块会展开相应工具组并定位事件，点击事件优先显示“原始记录”。需要像日志工作台一样查看全景时，点击“放大轨迹”；点击“恢复对话”回到并排布局。
...
| 搜索事件或工具 | 保留全程顺序，高亮匹配事件并展开命中的工具组 |
...
轨迹和步骤说明只描述可观察的请求与结果，不重建模型内部思考。USER、CONTEXT、ASSISTANT、PROGRAM、TOOL、RESULT、STOP 是界面的展示分类；例如 RESULT 是程序记录的回执，真实模型消息中的 `role=tool` 仍以原始记录为准。当前是第一阶段：同屏观察、固定步骤讲解和证据定位；连续任务对话与基于轨迹的教学问答将在后续阶段接入。
