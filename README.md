...
公开模式用浏览器 Cookie 隔离访客任务和轨迹，完成的记录保留 7 天；清除 Cookie 后无法找回旧任务。每名访客每天最多发起 12 次模型请求，全站默认每天最多 100 次，可用 `LOOP_PUBLIC_DAILY_REQUESTS` 调低或调高。达到上限时停止调用模型，不自动换用付费模型。Coding 练习和本地编辑器在公开模式下关闭。
