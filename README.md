...
保持终端运行，在浏览器打开 **[http://127.0.0.1:8877/](http://127.0.0.1:8877/)**。启动成功后，终端会显示网页地址和只读工作区。首页保持在任务总览，不会自动进入最新任务。左侧列出全部有效历史与当前运行，支持搜索和折叠；点击一项查看记录，点击“新任务”开始执行。首次使用没有历史记录，提交后才会生成轨迹。
...
在左侧“任务与回答”中查看答案，并对照 [示例笔记](workspace/agent-loop.md) 核对引文。`completed` 只表示循环正常结束，任务结果仍需你验收；`budget_exhausted` 表示模型请求额度耗尽、无法继续。

### 5. 看懂这次轨迹

任务与回答占主要宽度，右侧显示紧凑执行轨迹。每次模型请求独立分组，同批工具缩进显示，反馈行指出回执进入了哪次请求。任务侧栏与轨迹栏都可以折叠，并记住各自的选择；新任务不会强行展开它们。

图上只显示有实际记录支持的关系。点击步骤会优先展开“原始记录”，同时显示该步耗时与模型用量；“说明”和“源码”保留在次要入口。

| 页面位置 | 能看到什么 |
|---|---|
| 模型节点 → 原始记录 | 发给模型的消息和工具定义，以及它返回的回答、工具请求和 `finish_reason` |
| 工具节点 → 原始记录 | 实际工具参数、文件内容或错误回执 |
| 回执连线 / 步骤关联 | 跳到接收结果的那次模型请求，核对输入中的 `role=tool` 消息 |
| 完整事件记录 | 展开全部程序动作，包括准备上下文、选择执行器和停止原因 |
| 源码 | 该次运行保存的相关函数源码；新 Go 运行使用编译时嵌入的代码 |
| 完整事件记录 → 搜索 | 按事件、工具或文件名筛选记录；主图保留完整关系，匹配节点会高亮 |
| 导出 JSON | 下载完整的 `loop-run-<id>.json`，即使页面已筛选也会导出整次运行 |

例如，点击工具节点的“进入第 N 次请求的消息”，会直接定位到下一次模型输入中的那条 `role: "tool"` 回执。

模型节点显示服务商实际返回的 token 数，轨迹摘要显示各次请求的累计用量；输入、输出及缓存输入可在原始记录详情中查看，缓存属于输入子项，不重复加到总计。缺少用量时显示“未返回”，部分已知时注明覆盖次数。短工具操作用毫秒显示；这些是请求或工具耗时，不是单独测量的“思考耗时”。

“原始记录”显示本地文件路径、事件 ID 与 JSON Pointer（如 `/events/10/input/messages/5`），可点击“复制定位”后在编辑器中核对。运行期间显示实时快照并提示 `run.json` 尚未完成落盘；已发生事件追加在 `trace.jsonl` 中。

轨迹和步骤说明只描述可观察的请求与结果，不重建模型内部思考。当前是第一阶段：同屏观察、固定步骤讲解和证据定位；连续任务对话与基于轨迹的教学问答将在后续阶段接入。
