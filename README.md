...
模型行标出“模型轮次 N”：一次模型请求及它引发的工具处理属于同一轮；一次响应提出多个工具调用不会增加轮次。没有工具调用的最终回答仍算一次模型轮次。现有原始记录的 `turn` 字段记录模型请求计数，准备阶段为 0，页面不将它标为“第 0 轮”。一轮用户对话可能包含多次模型请求，因此不直接把 `turn` 当成对话轮次。
