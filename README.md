...
保持终端运行，在浏览器打开 **[http://127.0.0.1:8877/](http://127.0.0.1:8877/)**。启动成功后，终端会显示网页地址和只读工作区。首页保持在任务总览，不会自动进入最新任务。左侧列出全部有效历史与当前运行，支持折叠；点击一项查看记录，点击“新任务”开始执行。首次使用没有历史记录，提交后才会生成轨迹。
...
步骤条和耗时轨道同时显示：上方按顺序列出每个事件，下方按实际时间分布展示输入、模型、工具和程序操作，无需切换。每条事件也保留自己的耗时。点击概览色块会展开相应工具组并定位事件，点击事件优先显示“原始记录”。需要像日志工作台一样查看全景时，点击“放大轨迹”；点击“恢复对话”回到并排布局。桌面端可左右拖动两栏之间的分界线调整宽度，比例保存在当前浏览器；双击恢复默认，也可聚焦分界线后用左右方向键调整。折叠、放大及手机竖排时不显示拖动条。
...
| 导出 | 下载完整的 `loop-run-<id>.json`，即使页面已筛选也会导出整次运行 |
...
轨迹和步骤说明只描述可观察的请求与结果，不重建模型内部思考。USER、CONTEXT、ASSISTANT、PROGRAM、TOOL、RESULT、STOP 是界面的展示分类；例如 RESULT 是程序记录的回执，真实模型消息中的 `role=tool` 仍以原始记录为准。PROGRAM 表示 Loop 程序自身的调度步骤，例如把模型提出的工具调用交给只读执行器；原始事件的 `kind` 是 `control`，不是一个 `role=program` 的模型消息，也不额外增加模型调用。当前是第一阶段：同屏观察、固定步骤讲解和证据定位；连续任务对话与基于轨迹的教学问答将在后续阶段接入。
